  ...
  return (
    <aside className="relative hidden h-screen w-72 shrink-0 border-r border-white/10 bg-black/30 px-4 py-5 backdrop-blur lg:block">
      <div className="flex items-center gap-3 px-2">
        <img src="/logo.jpg" alt="BuyLottoX" className="h-12 w-12 rounded-xl object-cover shadow-neon" />
        <div>
          <div className="text-lg font-semibold tracking-tight">BuyLottoX</div>
          <div className="text-xs text-white/50">Neon Lottery Platform</div>
        </div>
      </div>

      <div className="mt-6 space-y-2">
        <Item to="/dashboard" label="Dashboard" />
        <Item to="/buy-ticket" label="Buy Ticket" />
        <Item to="/my-tickets" label="My Tickets" />
        {user?.role === 'ADMIN' && <Item to="/admin" label="Admin Panel" />}
      </div>
