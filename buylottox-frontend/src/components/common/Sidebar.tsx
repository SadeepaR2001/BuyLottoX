  ...
  return (
    <aside className="relative hidden h-screen w-72 shrink-0 border-r border-white/10 bg-black/30 px-4 py-5 backdrop-blur lg:block">
      ...
      <div className="mt-6 space-y-2">
        ...
        {user?.role === 'USER' && <Item to="/my-tickets" label="My Tickets" />}
        {user?.role === 'USER' && <Item to="/payments" label="Payments" />}
        {user?.role === 'ADMIN' && <Item to="/admin" label="Admin Dashboard" />}
        {user?.role === 'ADMIN' && <Item to="/admin/users" label="Users" />}
        {user?.role === 'ADMIN' && <Item to="/admin/payments" label="Payment Review" />}
        {user?.role === 'ADMIN' && <Item to="/admin/draws" label="Draws" />}
      </div>
